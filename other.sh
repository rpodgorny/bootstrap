...
netsh advfirewall firewall delete rule name=any protocol=tcp localport=22 || true
netsh advfirewall firewall add rule name=ssh protocol=tcp localport=22 dir=in action=allow
netsh advfirewall firewall delete rule name=any protocol=tcp localport=3389 || true
netsh advfirewall firewall add rule name=rdp protocol=tcp localport=3389 dir=in action=allow
netsh advfirewall firewall delete rule name=any protocol=tcp localport=5900 || true
